...
\end{lstlisting}
    ...
    \begin{solutionorbox}
        \begin{align*}
            ...
            \equiv \psi[\result \leftarrow n * aux][aux \leftarrow n\mod{2}+(n+1)\mod{2}] \\
            \equiv \psi[\result \leftarrow n * (n \mod{2} + (n+1)\mod{2})] \\
            \equiv abs(n) = n * (n\mod{2} + (n+1)\mod{2}) \\
            \equiv n = n(n\mod{2} + (n+1)\mod{2}) \\
            \equiv 1 = n\mod{2} + (n+1)\mod{2} \\
            n pair : 1 = 0+1 ==> 1 = 1 \\
            n impair : 1 = 1+0 ==> 1 = 1 \\ 
            \top \code{abs2} {\psi} est un triplet de Hoare valide.
        \end{align*}
    \end{solutionorbox}
